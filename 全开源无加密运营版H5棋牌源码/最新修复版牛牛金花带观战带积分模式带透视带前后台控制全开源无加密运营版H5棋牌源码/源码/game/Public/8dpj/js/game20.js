    ...
    for(var i=0;i<data.user.length;i++){
        ...
        if(data.allcard && indexuser!=1){
            ...
            if(indexuser > 6) {
                ...
                html=html+'<div class="card cardopen card'+card[user.index][0]+' card'+indexuser+'4 jiurenniuniu-index11" ></div> ';
                html=html+'<div class="card cardopen card'+card[user.index][1]+' card'+indexuser+'3 jiurenniuniu-index12" ></div>';
                html=html+'<div class="card cardopen card'+card[user.index][2]+' card'+indexuser+'2 jiurenniuniu-index13" ></div> ';
                html=html+'<div class="card cardopen card'+card[user.index][3]+' card'+indexuser+'1 jiurenniuniu-index14" ></div></div>';
            }
            else {
                html=html+'<div class="cardss cards card'+indexuser+'" style="display: none;" data="'+indexuser+'">';
                html=html+'<div class="card cardopen card'+card[user.index][0]+' card'+indexuser+'1 jiurenniuniu-index11" ></div> ';
                html=html+'<div class="card cardopen card'+card[user.index][1]+' card'+indexuser+'2 jiurenniuniu-index12" ></div>';
                html=html+'<div class="card cardopen card'+card[user.index][2]+' card'+indexuser+'3 jiurenniuniu-index13" ></div> ';
                html=html+'<div class="card cardopen card'+card[user.index][3]+' card'+indexuser+'4 jiurenniuniu-index14" ></div></div>';
            }
        }
        else{
            html=html+'<div class="cardss cards card'+indexuser+'" style="display: none;" data="'+indexuser+'">';
            html=html+'<div class="card card'+indexuser+'1 jiurenniuniu-index11" ></div>';
            html=html+'<div class="card card'+indexuser+'2 jiurenniuniu-index12" ></div>';
            html=html+'<div class="card card'+indexuser+'3 jiurenniuniu-index13" ></div>';
            html=html+'<div class="card card'+indexuser+'4 jiurenniuniu-index14" ></div></div>';
        }
        $('#userfp').append(html);
    }
    $('#userfp .cards').show();
...
function fapxx(data){
    ...
    $('.myCards .card'+(data.id-1)).attr("data-card", data.card).show();
...
function showtanpai(data){
    ...
    var msgxx={};
    msgxx.index=index;
    msgxx.img1='/themes/game/Public/zhongyi/img/pai/point/'+(data.win1 ? 'win' : 'loss')+data.cardType1+'.png';
    msgxx.img2='/themes/game/Public/zhongyi/img/pai/point/'+(data.win2 ? 'win' : 'loss')+data.cardType2+'.png';
    showmemberBull(msgxx);
    mp3xx='mp3niu'+getsex(index)+data.cardType1;
    mp3play(mp3xx);
    cardDoneResult(data);

    setTimeout(function () {
        $("div.memberBull1 img").show();
        mp3xx='mp3niu'+getsex(index)+data.cardType2;
        mp3play(mp3xx);
    }, 2000);
}
function showothertanpai(data){
    if(data.index==index){
        ...
        $(".memberTimesText"+indexuser).remove();
        var msgxx={};
        ...
        mp3play(mp3xx);
        setTimeout(function () {
            $("div.memberBull"+indexuser+" img").show();
            mp3xx='mp3niu'+getsex(index)+data.cardType2;
            mp3play(mp3xx);
        }, 2000);
    }
...
function tanpaixxother(data){
    ...
    var html='<div>';
    for(var i=0;i<data.card1.length;i++){
        var card=data["card1"][i];
        var cs = 'jiurenniuniu-index1'+(1-i)+'s'+(indexuser<6 ? 's' : '');
        html=html+'<div class="cards card'+indexuser+' card'+indexuser+(i-(-1))+'1 '+cs+' zh card-flipped"><div class="face front"></div> <div class="face back card'+indexuser+' card'+card+' card-flipped"></div></div>';
    }
    for(var i=0;i<data.card2.length;i++){
        var card=data["card2"][i];
        var cs = 'jiurenniuniu-index1'+(1-i)+'s'+(indexuser<6 ? 's' : '');
        html=html+'<div class="cards card'+indexuser+' card'+indexuser+(i-(-3))+'1 '+cs+'"><div class="face front"></div><div class="face back card'+indexuser+' cardundefined" data-card="'+card+'"></div></div>';
    }
    html=html+'</div>';
    ...
    setTimeout(function () {
        $('.cardOver .back.cardundefined').each(function (i, item) {
            $(item).removeClass("cardundefined").addClass("card"+$(item).attr("data-card")).addClass("card-flipped");
            $(item).parent().addClass("card-flipped");
        });
    }, 2000);
}
...
function showmemberBull(data){
    ...
    var html='<div class="memberBull'+indexuser+'" style="display: block;">' +
        '<img src="'+data.img1+'" style="position: absolute; left: 0; width: 50%;height: 100%;"/>' +
        '<img src="'+data.img2+'" style="position: absolute; left: 50%; width: 50%;height: 100%; display: none;"/>' +
        '</div>';
...
function jibichange(data){
    ...
    if(fx==0){
        setTimeout('jibiover2()',8000);
    }
    else{
        setTimeout('jibiover2()',6500);
    }
}
function jibiover2(){
    jibiTime = false;
...
function initroom(){
    ...
    $('.myCards').eq(0).html(' <div class="cards3D">' +
        ...
        '<div class="face front"></div>' +
        '<div class="face back cardundefined"></div></div>'+
        '<div class="cards card2" style="display: none; transition: left 1s;">' +
        '<div class="face front"></div>' +
        '<div class="face back cardundefined"></div></div>' +
        '<div class="cards card3" style="display: none;">' +
        '<div class="face front"></div>' +
        '<div class="face back cardundefined"></div></div></div>');
    ...
    $('.myCards').eq(2).html('<div class="cards card00"  >' +
        ...
        '<div class="cards card02" ><div class="face back cardundefined" style="transform:rotateY(0deg);-ms-transform:rotateY(0deg);-moz-transform:rotateY(0deg);-webkit-transform:rotateY(0deg);-o-transform:rotateY(0deg);"></div></div>'+
        '<div class="cards card03" ><div class="face back cardundefined" style="transform:rotateY(0deg);-ms-transform:rotateY(0deg);-moz-transform:rotateY(0deg);-webkit-transform:rotateY(0deg);-o-transform:rotateY(0deg);"></div></div>');
...
function operationButton(data){
    ...
    if(data=='10') {
        html+='<div class="operationButton-card-tips">' +
              '<img class="operationButton-1" src="/themes/game/Public/zhongyi/img/btn_hit1.png" onclick="cardTips()"/>'+
              '</div>';
        html+='<div class="operationButton-card-zh">' +
            '<img id="zupai1" class="operationButton-1" src="/themes/game/Public/zhongyi/img/zupai_btn2.png"/>'+
            '</div>';
    }
    if(data=='10') {
        $('.myCards .cards3D > .cards').click(function (e) {
            cardSelect($(this).index());
        });
    }
...
function cardSelect(id) {
    var $obj = $('.myCards .cards3D > .card'+id);
    if($obj.hasClass("cardSelect")) {
        $obj.removeClass("cardSelect");
    } else {
        if($(".myCards .cards3D > .cardSelect").length > 1) {
            $(".myCards .cards3D > .cardSelect:first").removeClass("cardSelect");
        }
        $obj.addClass("cardSelect");
    }
    var indexs = [];
    $('.myCards .cards3D > .cards').each(function (i, item) {
        if($(item).hasClass("cardSelect")) {
            indexs.push(i);
        }
    });
    if(indexs.length == 2) {
        $("#zupai1").attr("src", "/themes/game/Public/zhongyi/img/zupai_btn1.png");
        $("#zupai1").attr("onclick", "cardDone()");
        send("cardselect", {"indexs":indexs});
    } else {
        $("#zupai1").attr("src", "/themes/game/Public/zhongyi/img/zupai_btn2.png");
        $("#zupai1").attr("onclick", "");
    }
}
...
function cardDoneResultOther(data) {
    var indexuser=data-index-(-1);
    if(indexuser<=0){
        indexuser=indexuser-(-8);
    }
    $('.cardDeal .card'+indexuser+" > div.card:eq(0)").addClass("zh");
    $('.cardDeal .card'+indexuser+" > div.card:eq(1)").addClass("zh");
}

function cardDoneResult(data) {
    $(".myCards .cards3D > .cards").attr("style", "").removeClass("cardSelect");
    $(".myCards .cards3D > .cards:eq(0) .back").attr("class","face back card"+data.card1[0]);
    $(".myCards .cards3D > .cards:eq(1) .back").attr("class","face back card"+data.card1[1]);

    $(".myCards .cards3D > .cards:eq(2) .back").attr("class","face back card"+data.card2[0]);
    $(".myCards .cards3D > .cards:eq(3) .back").attr("class","face back card"+data.card2[1]);

    $(".myCards .cards3D > .cards:eq(2)").addClass("zh");
    $(".myCards .cards3D > .cards:eq(3)").addClass("zh");
}

function cardTipsSelect(data) {
    $(".myCards .cards3D > .cards").removeClass("cardSelect");
    var counts = {};
    for(var k in data) {
        counts[data[k]] = ! counts[data[k]] ? 0 : counts[data[k]]+1;
        var ik = 0;
        $(".myCards .cards3D > .cards").each(function (i, item) {
            if($(item).attr("data-card") == data[k] && ik == counts[data[k]]) {
                cardSelect(i);
                ik ++;
            }
        });
